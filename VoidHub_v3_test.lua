--[[
    VoidHub v3.1 | Omnissiah's Ascension
    Made by void_fworld
    Полный код на основе Forsaken Plus v1.66
    Улучшенный AntiStun, отправка в чат, все функции
]]
...
-- ===== УЛУЧШЕННЫЙ ANTI-STUN (полностью блокирует стан, замедление, паралич) ===== --
function ToggleAntiStun(Value)
    if Value then
        RunService.Heartbeat:Connect(function()
            ...
            local char = LocalPlayer.Character
            if not char then return end
            -- Удаляем все NumberValue с именами, содержащими "stun", "slow", "paralyze" и т.д.
            ...
            -- Также проверяем атрибуты персонажа
            ...
            -- Перехватываем попытки сервера применить стан через RemoteEvent (если есть)
            if Network then
                local remote = Network:FindFirstChildOfClass("RemoteEvent")
